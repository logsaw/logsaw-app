...
<xsl:template name="helpidx">
  ...
  <xsl:template match="d:indexterm|d:primary|d:secondary|d:tertiary" mode="idx">
	...
	<xsl:variable name="href">
	  <xsl:call-template name="href.target.with.base.dir">
		<xsl:with-param name="context" select="/"/>        <!-- Generate links relative to the location of root file/toc.xml file -->
		<xsl:with-param name="object" select="parent::d:indexterm"/>
	  </xsl:call-template>
	</xsl:variable>
	...
	<xsl:choose>
	  <xsl:when test="self::d:indexterm and generate-id((//d:indexterm[d:primary/text()=$primary])[1])=generate-id(.)">
		<xsl:apply-templates select="d:primary" mode="idx"/>
	  </xsl:when>
	  <xsl:when test="self::d:primary">
		<entry keyword="{$text}">
		  <topic href="{$href}"/>
		  <xsl:apply-templates select="//d:secondary[preceding-sibling::d:primary=$primary]" mode="idx"/>
		</entry>
	  </xsl:when>
	  <xsl:when test="self::d:secondary">
		<entry keyword="{$text}">
		  <topic href="{$href}"/>
